...
You can also use bracket notation to create array structures. Use `.[index]` to specify array indices:
...
ShadForm(
  child: Column(
    children: [
      ShadInputFormField(
        id: 'users.[0].name',
        label: const Text('First User Name'),
      ),
      ShadInputFormField(
        id: 'users.[0].email',
        label: const Text('First User Email'),
      ),
      ShadInputFormField(
        id: 'users.[1].name',
        label: const Text('Second User Name'),
      ),
      ShadInputFormField(
        id: 'users.[1].email',
        label: const Text('Second User Email'),
      ),
    ],
  ),
)
...
For arrays, provide the array structure directly:
...
ShadForm(
  ...
  child: // Your form fields with array notation IDs
)
```

The form will automatically extract values from the nested structure based on the field IDs. For example, a field with `id: 'user.name'` will get the value from `initialValue['user']['name']`, and `id: 'users.[0].name'` will get the value from `initialValue['users'][0]['name']`.
...
Array notation also works with custom separators:
