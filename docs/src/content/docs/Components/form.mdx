...
### Initial values with dot notation

You can also use dot notation in the `initialValue` map:
...
ShadForm(
  ...
  child: // Your form fields with the same dot notation IDs
)
```

### Customizing the separator

If you prefer a different separator, you can customize it using the `dotNotationSeparator` parameter:
...
If you want to use dots in your field IDs without creating nested structures, you can disable the feature:
...
With `enableDotNotation: false`, the form value will be:
```dart
{
  'user.email': 'john@example.com' // Flat structure
}
```
