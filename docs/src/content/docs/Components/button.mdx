---
title: Button
sidebar:
  order: 4
---

import Preview from "../../../components/Preview.astro";

Displays a button or a component that looks like a button.

## Primary

<Preview src="https://shadcn-ui-playground.pages.dev/button?style=primary">

```dart
ShadButton(
  child: const Text('Primary'),
  onPressed: () {},
)
```

</Preview>

## Secondary

<Preview src="https://shadcn-ui-playground.pages.dev/button?style=secondary">

```dart
ShadButton.secondary(
  child: const Text('Secondary'),
  onPressed: () {},
)
```

</Preview>

## Destructive

<Preview src="https://shadcn-ui-playground.pages.dev/button?style=destructive">

```dart
ShadButton.destructive(
  child: const Text('Destructive'),
  onPressed: () {},
)
```

</Preview>

## Outline

<Preview src="https://shadcn-ui-playground.pages.dev/button?style=outline">

```dart
ShadButton.outline(
  child: const Text('Outline'),
  onPressed: () {},
)
```

</Preview>

## Ghost

<Preview src="https://shadcn-ui-playground.pages.dev/button?style=ghost">

```dart
ShadButton.ghost(
  child: const Text('Ghost'),
  onPressed: () {},
)
```

</Preview>

## Link

<Preview src="https://shadcn-ui-playground.pages.dev/button?style=link">

```dart
ShadButton.link(
  child: const Text('Link'),
  onPressed: () {},
)
```

</Preview>

## Icon

<Preview src="https://shadcn-ui-playground.pages.dev/button?style=icon">

```dart
ShadButton.outline(
  icon: const ShadImage(LucideIcons.chevronRight),
  onPressed: () {},
),
```

</Preview>

## Text and Icon

<Preview src="https://shadcn-ui-playground.pages.dev/button?style=textIcon">

```dart
ShadButton(
  onPressed: () {},
  child: const Text('Login with Email'),
  icon: const ShadImage(LucideIcons.mail),
)
```

</Preview>

## Loading

<Preview src="https://shadcn-ui-playground.pages.dev/button?style=loading">

```dart
ShadButton(
  onPressed: () {},
  child: const Text('Please wait'),
  icon: const SizedBox.square(
    dimension: 16,
    child: CircularProgressIndicator(strokeWidth: 2),
  ),
)
```

</Preview>

## Gradient and Shadow

<Preview src="https://shadcn-ui-playground.pages.dev/button?style=gradientShadow">

```dart
ShadButton(
  onPressed: () {},
  gradient: const LinearGradient(colors: [
    Colors.cyan,
    Colors.indigo,
  ]),
  shadows: [
    BoxShadow(
      color: Colors.blue.withOpacity(.4),
      spreadRadius: 4,
      blurRadius: 10,
      offset: const Offset(0, 2),
    ),
  ],
  child: const Text('Gradient with Shadow'),
)
```

</Preview>

## Troubleshoot

### iconSize

The size of the icon, defaults to `Size.square(16)`.

:::caution
The icon size is applied only if you use a `ShadImage` as `icon`. If you use something else, wrap it with a `SizedBox`.
:::

### applyIconColorFilter

By default a color filter is applied to the icon to keep it in sync with the text color.

:::caution
If you are overriding the icon color and it doesn't work, set `applyIconColorFilter` to false
:::
